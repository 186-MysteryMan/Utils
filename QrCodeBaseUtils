...
//                    log.info("image path :{}", imaPath);
//                    is = new ClassPathResource(imaPath).getInputStream();
//                    log.info("file input stream size :{}", is.available());
//                    byte[] bb=new byte[is.available()];
//                    int readByteCount = is.read(bb);
//                    log.info("read byte count :{}", readByteCount);
//                    ByteArrayImageSource bais = new ByteArrayImageSource(bb);
//
//                    src = new ToolkitImage(bais);
//                    src = ImageIO.read(new ClassPathResource(imaPath).getInputStream());
                    src = Toolkit.getDefaultToolkit().getImage(QrCodeBaseUtils.class.getClassLoader().getResource(imaPath));
                }
                if (src != null) {
                    img = toBufferedImage(src);
                }
            } catch (Exception e) {
                log.error("转换为BufferedImage异常", e);
            }finally {
                CloseStreamUtil.close(is,"关闭海报读取流失败");
            }
        }
        return img;
    }
